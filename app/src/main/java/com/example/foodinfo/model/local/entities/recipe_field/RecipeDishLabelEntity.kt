...
@Entity(
    tableName = RecipeDishLabelEntity.TABLE_NAME,
    foreignKeys = [ForeignKey(
        entity = RecipeEntity::class,
        parentColumns = arrayOf(RecipeEntity.Columns.ID),
        childColumns = arrayOf(RecipeDishLabelEntity.Columns.RECIPE_ID),
        onUpdate = ForeignKey.CASCADE,
        onDelete = ForeignKey.CASCADE
    )]
)
data class RecipeDishLabelEntity(
    ...
    companion object {
        const val TABLE_NAME = "recipe_dish_labels"
        const val DISPLAY_NAME = "dish"
    }
}